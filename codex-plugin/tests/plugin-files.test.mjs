...
test("README explains global hooks and project overrides", () => {
  ...
  assert.match(readme, /most releases only need a restart/i);
  ...
  assert.match(readme, /--update-check enabled/);
  assert.match(readme, /updateCheck/);
  ...
  assert.match(readme, /\$CODEX_HOME\/mem9\/state\.json/);
