...
const temp = mkdtempSync(join(tmpdir(), "mem9-deepseek-pack-"));
...
try {
  ...
  if (!config.includes("@mem9/deepseek-plugin") || !config.includes("id: mem9")) {
  ...
  if (!profileManifest.dsh?.profile?.bundles?.includes("@mem9/deepseek-plugin")) {
    throw new Error("profile manifest did not list @mem9/deepseek-plugin as a bundle");
  }
  run(process.execPath, [dshBin, "plugin", "--profile", "mem9-pack-smoke", "remove", "@mem9/deepseek-plugin"]);
