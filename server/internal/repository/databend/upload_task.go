...
// FetchPending claims pending tasks without row-level locking.
// Databend does not support FOR UPDATE SKIP LOCKED, so we use an optimistic
// approach: UPDATE status to 'processing' first, then SELECT the claimed tasks.
func (r *UploadTaskRepoImpl) FetchPending(ctx context.Context, limit int) ([]domain.UploadTask, error) {
	...
	// Atomically claim pending tasks by updating their status.
	_, err := r.db.ExecContext(ctx,
		fmt.Sprintf(`UPDATE upload_tasks SET status = 'processing', updated_at = NOW() WHERE task_id IN (SELECT task_id FROM upload_tasks WHERE status = 'pending' ORDER BY created_at LIMIT %d)`, limit),
	)
	if err != nil {
		return nil, fmt.Errorf("claim pending upload tasks: %w", err)
	}

	// Fetch the tasks we just claimed. Use a narrow time window to avoid
	// picking up tasks from other workers.
	rows, err := r.db.QueryContext(ctx,
		`SELECT `+uploadTaskColumns+` FROM upload_tasks WHERE status = 'processing' ORDER BY created_at LIMIT ?`,
		limit,
	)
	if err != nil {
		return nil, fmt.Errorf("fetch claimed upload tasks: %w", err)
	}
	defer rows.Close()
