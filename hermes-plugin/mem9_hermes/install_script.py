...
def install_plugin(hermes_dir, dry_run=False):
    """安装 mem9 插件"""
    print("\n📦 开始安装 mem9 插件...\n")
    
    # 获取包所在目录（支持 pip 安装和本地运行两种模式）
    try:
        import mem9_hermes
        package_dir = Path(mem9_hermes.__file__).parent.parent
    except ImportError:
    ...
    src = package_dir / "plugins" / "memory" / "mem9"
    ...
    src = package_dir / "agent" / "providers" / "mem9_provider.py"
    ...
    src = package_dir / "tools" / "mem9_tools.py"
