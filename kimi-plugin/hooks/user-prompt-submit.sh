...
# UserPromptSubmit is the only Kimi hook whose stdout reaches the model: print plain text only.
...
if [[ "${load_auth_status}" -ne 0 ]]; then
  if [[ "${load_auth_status}" -eq 3 ]]; then
    mem9_debug "UserPromptSubmit" "auth_ambiguous"
...
prompt="$(mem9_hook_get_prompt "${HOOK_INPUT}")"
if [[ -z "${prompt}" ]]; then
