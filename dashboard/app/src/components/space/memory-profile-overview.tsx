...
export function MemoryProfileOverview({ spaceId, stats, memories, cards, snapshot, range, matchMap, facetSummary, loading, className }: { spaceId: string; stats: MemoryStats | undefined; memories: Memory[]; cards: AnalysisCategoryCard[]; snapshot: AnalysisJobSnapshotResponse | null; range: TimeRangePreset; matchMap: Map<string, MemoryAnalysisMatch>; facetSummary: TopicSummary | undefined; loading: boolean; className?: string }) {
  ...
  const radarTopics = useMemo(
    () => buildRadarTopics(cards, memories, matchMap),
  ...
  return <section data-testid="memory-profile-overview" className={cn("relative", className)} style={{ animation: "slide-up 0.45s cubic-bezier(0.16,1,0.3,1)" }}>
    ...
    <div className={cn("mt-4", profileGridClass)}>
      <ProfileCard title={t("memory_profile.topics.title")}><RadarChart topics={radarTopics} /></ProfileCard>
...
export function buildRadarTopics(
  ...
  for (const memory of memories) {
    for (const category of matchMap.get(memory.id)?.categories ?? []) {
      matchedCounts.set(category, (matchedCounts.get(category) ?? 0) + 1);
    }
  }
  ...
  return cards
    ...
    .map((card) => ({
      ...
      label: card.category,
      count: Math.max(card.count, matchedCounts.get(card.category) ?? 0),
    }))
    .sort((left, right) =>
...
function RadarChart({ topics }: { topics: RadarTopic[] }) {
  ...
  const path = topics.length > 1
    ? `${topics.map((_, index) => `${index === 0 ? "M" : "L"}${points[index]![0]} ${points[index]![1]}`).join(" ")}${topics.length > 2 ? " Z" : ""}`
    : "";
  ...
  return <svg viewBox="0 0 260 190" className="profile-radar mx-auto h-[190px] w-full max-w-[280px]" aria-label="Memory Insight topics">
    ...
    {topics.map((topic, index) => { const [x, y] = points[index]!; const label = labels[index]!; const active = hoveredIndex === index; return <g key={topic.id} tabIndex={0} role="img" aria-label={`${topic.label}: ${topic.count}`} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} onFocus={() => setHoveredIndex(index)} onBlur={() => setHoveredIndex(null)} className="cursor-pointer"><text x={label.x} y={label.y} textAnchor={label.anchor} className={cn("text-[10px] font-medium transition-all duration-200", active ? "fill-blue-500" : "fill-muted-foreground")} style={{ transform: active ? "translateY(-2px)" : "translateY(0)" }}><tspan x={label.x}>{topic.label}</tspan><tspan x={label.x} dy={label.countDy} className={cn("text-[9px] tabular-nums transition-colors duration-200", active ? "fill-blue-500/75" : "fill-foreground/60")}>{topic.count}</tspan></text><circle cx={x} cy={y} r={active ? 12 : 7} fill="#3b82f6" opacity={active ? .2 : 0} className="transition-all duration-200" /><circle className="profile-radar-node transition-all duration-200" cx={x} cy={y} r={active ? 6 : 4} fill="#3b82f6" /><title>{`${topic.label}: ${topic.count}`}</title></g>; })}
  </svg>;
}
