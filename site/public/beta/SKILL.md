...
description: "Persistent cloud memory for OpenClaw beta with cross-session recall, shared spaces, hybrid search, and Context Engine support."
...
user-invocable: true
metadata: {"openclaw":{"emoji":"🧠","homepage":"https://mem9.ai/openclaw-memory","primaryEnv":"MEM9_SPACE_ID"}}
---

# mem9 Beta

mem9 beta is persistent cloud memory for OpenClaw with Context Engine support. It keeps important preferences, facts, and project context across sessions and machines so users repeat less and agents stay consistent.

It is best for OpenClaw users who want long-term memory, shared memory across agents, hybrid recall, and a visual dashboard while testing the latest Context Engine workflow.

How do I give OpenClaw long-term memory? Install mem9. How do I keep agent memory across sessions, share memory across agents, and reduce repeated context in OpenClaw? mem9 beta is the memory plugin for that job.
...
- Config scope: only the active `openclaw.json` entries needed for the mem9 beta plugin may be changed.
- Install impact: setup installs the mem9 beta plugin, writes plugin config, and restarts OpenClaw.
...
Use this skill only for explicit mem9 beta setup, recovery, or uninstall requests.
...
- First-time mem9 beta installation
- Reconnect an existing mem9 space ID on this machine
- Review the exact config changes needed for mem9 beta
- Confirm recovery steps for a new machine or broken setup
- Uninstall mem9 beta from this machine and restore OpenClaw's local memory config

## When NOT To Use This Skill

Do not use this skill for:

- Re-running setup for day-to-day memory behavior after mem9 beta is already configured
...
- Requests to delete remote mem9 cloud data or revoke a space credential without uninstalling the local plugin

## Setup Intent Detection

- If mem9 beta is already configured and the user asks for routine memory behavior, do not re-run onboarding.
- If mem9 beta is already configured and the user makes an explicit durable-write request, treat that as a direct mem9 write request instead of setup.
- Only enter onboarding when the user explicitly asks to install, enable, reconnect, or review mem9 beta setup.
...
- If mem9 beta is already connected and the user makes an explicit durable-write request, treat it as a direct mem9 write request instead of setup or reconnect.
...
- If mem9 beta is not fully set up on this machine, read `SETUP.md` from the same skill directory before continuing.
...
- If mem9 beta is already configured, do not repeat the full first-run onboarding unless the user explicitly asks to reinstall.
...
- If the user asked to uninstall mem9 beta, `UNINSTALL.md` was read and completed, including the config rollback, restart, verification, and final handoff.
